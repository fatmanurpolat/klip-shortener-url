...
/** Klipo text field — label, optional addon/icon, helper/error, rose focus bloom. */
export function Input({ label, hint, error, prefix, iconLeft, size = "md", id, style, wrapStyle, ...rest }: InputProps) {
  const autoId = useId();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "6px", ...wrapStyle }}>
      ...
      >
        {prefix && (
          <span style={{ paddingLeft: "14px", color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontSize: "var(--text-sm)", whiteSpace: "nowrap" }}>
            {prefix}
          </span>
        )}
      ...
      {(hint || error) && (
        <span
          ...
          style={{ fontFamily: "var(--font-body)", fontSize: "var(--text-xs)", color: error ? "var(--blush-700)" : "var(--text-muted)" }}
        >
          {error || hint}
        </span>
      )}
